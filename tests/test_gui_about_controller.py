...
def test_python_version(about_ctrl: AboutController) -> None:
    ...
    assert about_ctrl.pythonVersion == sys.version.split()[0]


def test_runtime_versions(about_ctrl: AboutController) -> None:
    """Qt/PySide2/dulwich 版本非空。."""
    assert about_ctrl.qtVersion
    assert about_ctrl.pyside2Version
    assert about_ctrl.dulwichVersion


def test_license_text(about_ctrl: AboutController) -> None:
    """开源许可为 MIT。."""
    assert about_ctrl.licenseText == "MIT License"
